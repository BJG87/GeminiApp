...
The job queue allows you to process tasks in the background using time-driven triggers.

### Setup

1. **Create a processJobs function** in your project:

```javascript
/**
 * Process jobs from the queue
 * This function will be called by a time-driven trigger
 */
...
2. **Implement your job processor**:

You need to create a function that handles your specific job types:

```javascript
/**
 * YOUR custom job processor
 * This is called by the library for each job
 */
function processJob(jobData) {
  ...
  switch(jobData.type) {
    ...
    case 'analyze':
      const analysis = ai.prompt(
        'Analyze: ' + jobData.text,
        { schema: { sentiment: 'string', score: 'number' } }
      );
      // Save analysis somewhere
...
```javascript
// Add a single job
const jobId = StvpsAi.addJob({
  type: 'transcribe',
  fileId: 'DRIVE_FILE_ID'
});

// Add multiple jobs
const jobIds = StvpsAi.addJobs([
  { type: 'transcribe', fileId: 'FILE_1' },
  { type: 'transcribe', fileId: 'FILE_2' },
  { type: 'analyze', text: 'Some text' }
]);

// Start processing (creates a trigger)
StvpsAi.startJobQueue();

// Set concurrent jobs (default: 1)
StvpsAi.setMaxConcurrentJobs(3); // Process 3 jobs simultaneously
